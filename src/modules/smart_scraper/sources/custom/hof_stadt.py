...
class HofStadtSource(BaseSource):
    ...
    def _parse_event(self, container) -> Optional[Dict[str, Any]]:
        ...
        return {
            'id': f"hofstadt_{hash(title + start_time)}",
            'title': title[:200],
    ...
    def _extract_date(self, text: str) -> str:
            ...
            if match:
                try:
                    if format_type == 'DMY':
                        ...
                        date = datetime(int(year), int(month), int(day), 10, 0)
                    elif format_type == 'DMY_SHORT':
                        day, month, year = match.groups()
                        year = int('20' + year) if int(year) < 50 else int('19' + year)
                        date = datetime(year, int(month), int(day), 10, 0)
        ...
        return (datetime.now() + timedelta(days=7)).replace(hour=10, minute=0).isoformat()
